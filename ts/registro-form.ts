...
function validateRegistroNombre(value: string): string {
...
function validateRegistroEmail(value: string): string {
  const trimmed = value.trim();
  if (!trimmed) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const passwordConfirm = form.querySelector<HTMLInputElement>(
    "#password-confirm"
  );
  const terminos = form.querySelector<HTMLInputElement>("#terminos");
  ...
  const validateTerminos = (): string =>
    terminos.checked ? "" : "Debes aceptar los términos para continuar.";

  const fields: RegistroFieldConfig[] = [
  ...
  const showFieldError = (field: RegistroFieldConfig, message: string): void => {
  ...
  const validateField = (field: RegistroFieldConfig): boolean => {
    const message =
      field.input === terminos
        ? validateTerminos()
  ...
  fields.forEach((field) => {
    const eventName =
      field.input.type === "checkbox" ? "change" : "blur";
